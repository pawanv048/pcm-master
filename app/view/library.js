...
class Library extends React.Component {
            ...
            <Screen statusBarTint='white' onFocus={this.onFocus}>
                ...
                <Body scroll loading={this.state.loading} extraStyle={[{ paddingTop: 0, paddingHorizontal: 0, paddingBottom: 0, backgroundColor: 'white' }]}>
                    ...
                    <Col extraStyle={[{ paddingHorizontal: 20, paddingBottom: 20, alignItems: 'stretch' }]}>
                        ...
                        {this.state.parentCategory != null ?
                            this.state.parentCategory.detail !== '' ?
                                <Text style={[stylesC.textM14, {marginTop:5}]}>
                                  {this.state.parentCategory.detail}
                                </Text>
                                :
                                null
                                ...
                                <ColButton
                                    ...
                                    <Row>
                                        ...
                                        <Col extraStyle={[{ flex: 1, alignSelf: 'center' }]}>
                                            ...
                                            {model.videos > 0 ?
                                                this.checkHasSubCategories(model) ?
                                                    null
                                                    :
    ...
    onCategoryPress = (model) => {
        ...
        if (subCategories.length > 0) {
            // load sub categories
            this.props.setSelectedCategory(model)
    ...
    loadSubcategories = () => {
        setTimeout(() => {
            ...
            if (parent == null) {
                ...
                this.setState({ models: this.getParentCats(this.props.categories) })
            }
            else {
                // load subcategories
                ...
                this.setState({ models: subCategories })
            }
        }, 50);
    };
    ...
    onFocus = () => {
        console.log('onFocus');
        let parentCategory = this.props.selectedCategory
        console.log('parentCategory: ' + JSON.stringify(parentCategory));
        if (parentCategory != null)
            this.setState({ parentCategory: parentCategory })
        else
            this.setState({ parentCategory: null, models: [] })
        this.loadSubcategories()
        // this.props.setSelectedCategory(null) // clear opening category
    };
...
const mapStateToProps = (state) => {
    return {
        ...
        selectedCategory: state.SelectedCategoryReducer.selectedCategory,
