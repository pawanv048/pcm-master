import React from 'react';
import { Text, View, FlatList, TextInput, KeyboardAvoidingView, Platform, TextInputBase } from 'react-native';
import { stylesC } from '../styles/style_common.js';
import { Screen, Col, Line, Body, Row, IconCustomButton, TitleBar, Button, EditText, showToast, showErrorToast } from '../custom/components.js';
import * as Colors from '../constants/colors.js';
import * as Strings from '../constants/strings.js';
import { Input } from 'react-native-elements';
import Utils from '../util/utils.js';
import Api from '../api/api'
import { connect } from 'react-redux';
import { Actions } from '../redux/Actions'
import WeekdayPicker from '../util/weekday_picker'

const defaultExercise = { title: '', sets: '', video_id: '' }

class AddProgram extends React.Component {
    static navigationOptions = {
        headerShown: false
    };

    state = {
        loading: false,
        title: '',
        weekday: '',
        weekDays: [
            Strings.t1, Strings.t2, Strings.t3, Strings.t4, Strings.t5, Strings.t6, Strings.t7, Strings.t8
        ],
        fullWeekDays: [
            Strings.t1, Strings.t9, Strings.t10, Strings.t11, Strings.t12, Strings.t13, Strings.t14, Strings.t15
        ],
        exercises: [
            defaultExercise,
        ],
        settingIndex: 0, // unused
        program: null, // edit mode
    }

    render() {
        if (this.props.user == null) {
            this.props.navigation.navigate('Login');
            return null
        }
        return (
            <Screen statusBarTint='white' onFocus={this.onFocus}>
                <WeekdayPicker
                    ref={(ref) => this.weekdayPicker = ref}
                    onSelect={(index) => {
                        console.log('selected: ' + index);
                        this.setState({ weekday: this.state.weekDays[index] })
                    }} />
                <TitleBar
                    canUpdate
                    left
                    leftType='back' // back/menu
                    onLeftPress={this.onLeftPress}
                    title={this.state.program == null ? Strings.t16 : Strings.t17}
                    titlePos='flex-start' />
                <KeyboardAvoidingView
                    behavior={Platform.OS === 'ios' ? 'padding' : null}
                    style={{ flex: 1 }}>
                    <Body
                        loading={this.state.loading}
                        extraStyle={[{ paddingHorizontal: 0, paddingVertical: 0 }]}>
                        <Col extraStyle={[{ backgroundColor: 'white', marginTop: 0, borderRadius: 10, paddingHorizontal: 20, marginTop: 0, marginBottom: 0, borderWidth: 0, borderColor: Colors.lineLight }]}>
                            <View style={{ width: '100%', marginTop: 0 }}>
                                <FlatList
                                    showsVerticalScrollIndicator={false}
                                    horizontal={false}
                                    data={this.state.exercises}
                                    extraData={this.state} // refresh list on state change
                                    keyExtractor={(item, index) => index.toString()}
                                    renderItem={this._renderExercisesItem}
                                    ListHeaderComponent={this.showHeader}
                                    ListFooterComponent={this.showListFooter} />
                            </View>
                        </Col>
                    </Body>
                </KeyboardAvoidingView>
            </Screen>
        );
    }

    showHeader = () => {
        return (
            <Col extraStyle={[{ alignItems: 'stretch', marginBottom: 10, marginTop: 15 }]}>
                <EditText
                    reference={(input) => this.title = input}
                    extraStyle={{ paddingHorizontal: 0, marginBottom: 0, width: '100%' }}
                    placeholder={Strings.t18} // placeholder/label
                    value={this.state.title}
                    keyboardType='default' // number-pad, decimal-pad, numeric, email-address, phone-pad
                    onChangeText={(text) => {
                        this.setState({ title: text })
                    }}
                    blurOnSubmit
                    onSubmitEditing={() => {
                        //
                    }} />
                <Button
                    canUpdate
                    label={this.state.weekday === '' ? Strings.t19 : this.state.weekday}
                    activeOpacity={0.6}
                    buttonStyle={[stylesC.buttonO30, { marginTop: 10, marginHorizontal: 0, height:40 }]}
                    labelStyle={[stylesC.buttonOT14]}
                    onPress={this.chooseWeekday} />
            </Col>
        )
    };

    chooseWeekday = () => {
        this.weekdayPicker.showPopup(this.state.fullWeekDays)
    };

    showListFooter = () => {
        return (
            <Col center>
                <Col center>
                    <IconCustomButton
                        conatinerStyle={[stylesC.center, { width: '10%', marginBottom: 10 }]}
                        imageStyle={[stylesC.imageM30, { width: 40, height: 40, tintColor: Colors.green, alignSelf: 'center' }]}
                        resizeMode='contain'
                        source={require('../assets/add.png')}
                        onPress={this.addExercise} />
                </Col>
                <Button
                    canUpdate
                    label={Strings.t20}
                    activeOpacity={0.6}
                    buttonStyle={[stylesC.button45, { marginTop: 5, marginBottom: 20 }]}
                    labelStyle={[stylesC.buttonT16]}
                    onPress={this.saveExercises} />
            </Col>
        )
    };

    _renderExercisesItem = ({ item, index }) => {
        // console.log('_renderField index: '+index);
        return (
            <Col center>
                <Row spaceBetween>
                    <Text style={[stylesC.textD16, { marginBottom: 15, marginTop: 5, color: Colors.theme }]}>
                        {Strings.t21} {index + 1}
                    </Text>
                    <IconCustomButton
                        conatinerStyle={[stylesC.center, { width: '10%', marginBottom: 10 }]}
                        imageStyle={[stylesC.imageM24, { tintColor: Colors.red }]}
                        resizeMode='contain'
                        source={require('../assets/remove.png')}
                        onPress={() => {
                            this.removeExercise(item, index);
                        }} />
                </Row>
                <Col extraStyle={[{ marginTop: 10, marginBottom: 0 }]}>
                    <Text style={[stylesC.textM12]}>
                        {Strings.t22}
                    </Text>
                    <Row extraStyle={[{ marginTop: 5 }]}>
                        <Col extraStyle={[{ flex: 1, alignItems: 'stretch', borderColor: Colors.textLight, borderBottomWidth: 1, }]}>
                            <TextInput
                                autoCapitalize='none'
                                style={{ fontSize: 15, height: 40, backgroundColor: '#fafafa', paddingHorizontal: 10 }}
                                placeholderTextColor={Colors.textLight}
                                placeholder={''} // placeholder/label
                                value={item.video_id}
                                keyboardType='default' // number-pad, decimal-pad, numeric, email-address, phone-pad
                                onChangeText={(text) => {
                                    this.updateField('video_id', item, index, text)
                                }}
                                blurOnSubmit={true}
                                onSubmitEditing={() => {
                                    //
                                }} />
                        </Col>
                        <Button
                            label={Strings.t23}
                            activeOpacity={0.6}
                            buttonStyle={[stylesC.buttonO45, { marginLeft: 10, height: 40, width: 100 }]}
                            labelStyle={[stylesC.buttonOT14]}
                            onPress={() => {
                                this.item = item
                                this.index = index
                                this.props.navigation.navigate('ChooseVideo', { from: 'AddProgram' });
                                // Utils.moveToScreen(this.props.navigation, 'HomeStack', 'ChooseVideo')
                            }} />
                    </Row>
                </Col>
                <Col extraStyle={[{ marginTop: 10, borderColor: Colors.textLight, borderBottomWidth: 1 }]}>
                    <Text style={[stylesC.textM12]}>
                        {Strings.t24}
                    </Text>
                    <TextInput
                        autoCapitalize='words'
                        style={{ marginHorizontal: 0, flex: 1, fontSize: 15, width: '100%', height: 40, backgroundColor: '#fafafa', marginTop: 5, paddingHorizontal: 10 }}
                        placeholderTextColor={Colors.textLight}
                        placeholder={''} // placeholder/label
                        value={item.title}
                        keyboardType='default' // number-pad, decimal-pad, numeric, email-address, phone-pad
                        onChangeText={(text) => {
                            this.updateField('title', item, index, text)
                        }}
                        blurOnSubmit={true}
                        onSubmitEditing={() => {
                            //
                        }} />
                </Col>
                <Col extraStyle={[{ marginTop: 10, borderColor: Colors.textLight, borderBottomWidth: 1, marginBottom: 15 }]}>
                    <Text style={[stylesC.textM12]}>
                        {Strings.t25}
                    </Text>
                    <TextInput
                        style={{ marginHorizontal: 0, flex: 1, fontSize: 15, width: '100%', height: 40, backgroundColor: '#fafafa', marginTop: 5, paddingHorizontal: 10 }}
                        placeholderTextColor={Colors.textLight}
                        placeholder={Strings.t26+'. 10'} // placeholder/label
                        value={item.sets+''}
                        keyboardType='number-pad' // number-pad, decimal-pad, numeric, email-address, phone-pad
                        onChangeText={(text) => {
                            this.updateField('sets', item, index, text)
                        }}
                        blurOnSubmit={true}
                        onSubmitEditing={() => {
                            //
                        }} />
                </Col>
            </Col>
        )
    }

    addExercise = () => {
        this.setState(state => {
            const exercises = state.exercises.concat(defaultExercise);
            return { exercises: exercises };
        });
    };

    updateField = (fieldName, item, index, text) => {
        this.setState(state => {
            const exercises = state.exercises.map((exercise, i) => {
                if (i === index) {
                    console.log('edit: ' + i);
                    // edit
                    return {
                        ...exercise, // ...item
                        [fieldName]: text
                    };
                }
                else {
                    return exercise;
                }
            });
            return { exercises: exercises };
        });
    };

    removeExercise = (item, index) => {
        // if (this.state.exercises.length == 1) return;
        this.setState(state => {
            const exercises = state.exercises.filter((item, i) => {
                return index !== i; //false will delete
            });
            return { exercises: exercises };
        });
    };

    saveExercises = () => {
        if (this.state.title === '') {
            showErrorToast(Strings.t27)
            return
        }
        if (this.state.weekday === '') {
            showErrorToast(Strings.t28)
            return
        }
        // check if anything empty in exercises
        for (let i = 0; i < this.state.exercises.length; i++) {
            const exercise = this.state.exercises[i];
            if (exercise.title === '' || exercise.sets === '' || exercise.video_id === '') {
                showErrorToast(Strings.t29)
                return;
            }
            if (parseInt(exercise.sets) > 10) {
                showErrorToast(Strings.t30)
                return;
            }
        }

        this.setState({ loading: true });
        let masterId = Utils.randomFixedNumber(6)
        let userId = this.props.user.id
        let title = this.state.title
        let weekday = this.state.weekday
        let exercises = []
        for (let i = 0; i < this.state.exercises.length; i++) {
            const exercise = this.state.exercises[i];
            exercises = exercises.concat({
                title: exercise.title,
                sets: exercise.sets,
                video_id: exercise.video_id,
            })
        }
        console.log('ex: ' + JSON.stringify(exercises));

        if (this.state.program == null) {
            // add program
            Api.addProgram(masterId, userId, title, weekday, JSON.stringify(exercises),
                (response) => {
                    console.log('addProgram: ' + JSON.stringify(response));
                    this.setState({ loading: false })
                    if (response.status === 'success') {
                        showToast(Strings.t31)
                        this.props.navigation.goBack()
                    }
                    else {
                        showErrorToast(response.msg)
                    }
                },
                (error) => {
                    this.setState({ loading: false });
                    showErrorToast('' + error);
                });
        }
        else {
            // update
            Api.updateProgram(this.state.program.id, JSON.stringify(exercises), title, weekday,
                (response) => {
                    console.log('updateProgram: ' + JSON.stringify(response));
                    this.setState({ loading: false })
                    if (response.status === 'success') {
                        showToast(Strings.t32)
                        this.props.navigation.goBack()
                    }
                    else {
                        showErrorToast(response.msg)
                    }
                },
                (error) => {
                    this.setState({ loading: false });
                    showErrorToast('' + error);
                });
        }

    };

    onLeftPress = () => {
        this.props.navigation.goBack();
    }

    componentDidMount() {
        console.log('componentDidMount')
        const program = this.props.navigation.getParam('program');
        if (program) { // edit mode
            this.setState({
                program: program,
                title: program.title,
                weekday: program.weekday,
            })
            this.loadProgramExercises(program)
        }
    }

    onFocus = () => {
        console.log('onFocus'); 
        const copiedVideoID = this.props.navigation.getParam('copiedVideoID');
        const videoTitle = this.props.navigation.getParam('videoTitle');
        if (copiedVideoID) {
            this.updateField('video_id', this.item, this.index, copiedVideoID)
            setTimeout(() => {
                this.updateField('title', this.item, this.index, videoTitle)
            }, 50);
        }
    };

    loadProgramExercises = (program) => {
        this.setState({ loading: true });
        Api.getProgramExercises(program.id,
            (response) => {
                this.setState({ loading: false })
                if (response.status === 'success') {
                    let exercises = response.data
                    if (exercises.length != 0) {
                        this.setState({ exercises: exercises })
                    }
                }
            },
            (error) => {
                this.setState({ loading: false });
                showErrorToast('' + error);
            });
    };

}

const mapStateToProps = state => {
    return {
        user: state.UserReducer.user
    }
}
export default connect(mapStateToProps, {})(AddProgram)
